...
contract ZINAXDAO is BEP20{
    // Max supply: 50,000,000 ZINAX.
    uint256 public constant MAX_SUPPLY = 50000000 ether;
    // Initial Supply : 30,000,000 ZINAX
    uint256 public initialSupply = 30000000 ether;
    ...
    uint256 public swapThreshold =  5000* 10**uint256(18);
    ...
    // Max token transfer amount
    uint256 public maxTransferAmount = 50000 * 10**uint256(18);
    ...
    event SetShares(uint256 liquidityShare, uint256 burnShare);
    event SetSwapBackSettings(bool enabled, bool swapAll, uint256 amount);
    event SetTransferGas(uint256 newGas, uint256 oldGas);
    event SetMaxTransferAmount(uint256 newAmount, uint256 oldAmount);
        ...
        if (maxTransferAmount > 0) {
            if (
                ...
                require(
                    ...
                    "ZINAX::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    constructor(address owner) public BEP20("ZINAX DAO", "ZINAX", owner) {
        ...
        isWhitelisted[deadWallet] = true; 
        _excludedFromAntiWhale[owner] = true;
    }

    function mint(address _to, uint256 _amount) public onlyMinter {
        require(totalSupply() + _amount <= MAX_SUPPLY, "You have reached the maximum supply of this token");
        _mint(_to, _amount);
    }
    ...
    function _swapBack(uint256 tokenAmount) private {
        ...
        uint256 burnTokens = (tokenAmount * burnShare) / totalShares;
        uint256 amountToSwap = tokenAmount - liquidityTokens - burnTokens;
        uint256 balanceBefore = address(this).balance;

        super._transfer(address(this), deadWallet, burnTokens); // burn tokens
    ...
    function recoverBEP20(IBEP20 token, address recipient) external onlyOwner {
        require(address(token) != address(this), "Can't withdraw ZINAX");
    ...
    function setShares(uint256 newLiquidityShare, uint256 newBurnShare) external onlyOwner {
        liquidityShare = newLiquidityShare;
        burnShare = newBurnShare;
        totalShares = liquidityShare + burnShare;
        emit SetShares(liquidityShare, burnShare);
    }

    function setMaxTransferAmount(uint256 amount) external onlyOwner {
        uint256 tokenAmount = amount * 10**uint256(18);
        emit SetMaxTransferAmount(maxTransferAmount, tokenAmount);
        maxTransferAmount = tokenAmount;
    }
